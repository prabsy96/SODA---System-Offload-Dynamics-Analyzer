...
def replay_kernel_from_cpu_op(cpu_op, exp_kernel_name, metadata, kernel_idx, runs=1, warmup_runs=100):
    ...
    kernel_traces_dir = utils.get_path("PYTORCH_KERNEL_TRACES_DIR")
    ...
    event_sequences = utils.link_sequences(events)
...
def replay_all_event_sequences(event_sequences, env_metadata, runs=1, warmup_runs=100):
    ...
    for i, event_sequence in enumerate(event_sequences):
        ...
        exp_kernel_name = utils.clean_kernel_name(event_sequence['kernel']['name'])
...
def run_replay_pipeline(runs=1, warmup_runs=100):
    ...
    # Step 1: Load event sequences and env_metadata
    input_file = utils.get_path("PYTORCH_UNIQUE_KERNELS")
    data = utils.load_json(input_file)
    
    env_metadata_file = utils.get_path("PYTORCH_ENV_METADATA")
    env_metadata = utils.load_json(env_metadata_file)
