...
def save_replayed_sequences(event_sequences, env_metadata, output_dir):
    """Save replayed event sequences."""
    save_output(output_dir, "replayed_gemm_kernel_sequences.json", event_sequences, env_metadata)
...
def run_replay_pipeline(input_file, runs=1, warmup_runs=100):
    ...
    save_replayed_sequences(replayed_gemm_sequences, env_metadata, output_dir)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Replay event sequences from extracted metadata")
    parser.add_argument("input_file", help="Input JSON file with event sequences (e.g., unique_gemm_kernel_sequences.json)")
